$(function () {
    ...
    (function () {
        $(".monitor .tabs a").on("click", function () {
            // 点击当前的a 给当前的a 添加类(高亮)，同时让它的兄弟移除高亮类
            $(this).addClass("active").siblings("a").removeClass("active");
            // 当前a的索引
            var index = $(this).index();
            // 当前a的索引给内容，点当前a让与之对应的内容显示 ，让内容的兄弟隐藏 
            $(".monitor .content").eq(index).show().siblings(".content").hide();
        })

        //1  先克隆marquee里面所有的行（row）再把克隆的行加到marquee里面行的后面
        $(".marquee-view .marquee").each(function (index, value) {
